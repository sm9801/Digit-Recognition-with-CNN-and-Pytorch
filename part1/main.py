...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    test_y_m3 = update_y(train_y, test_y)[1]
    test_error = compute_test_error_mod3(test_x, test_y_m3, theta, temp_parameter)
    return test_error
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # YOUR CODE HERE
    train_x, train_y, test_x, test_y = get_MNIST_data()
    train_y_m3, test_y_m3 = update_y(train_y, test_y)
    theta, cost_function_history = softmax_regression(train_x, train_y_m3, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    test_error = compute_test_error(test_x, test_y_m3, theta, temp_parameter)
    return test_error
    raise NotImplementedError

print("Error Rate for Mod 3 Data: ", run_softmax_on_MNIST_mod3(temp_parameter = 1))
...
plot_cost_function_over_time(cost_function_history)
print('18 PCS Error Rate: ', test_error)
...
centered_X, fm = center_data(train_x)
m1 = principal_components(centered_X)
train_pca10 = project_onto_PC(train_x, pcs = m1, n_components = 10, feature_means = fm)
test_pca10 = project_onto_PC(test_x, pcs = m1, n_components = 10, feature_means = fm)
...
test_error2 = compute_test_error(test_cube, test_y, theta, temp_parameter = 1)
plot_cost_function_over_time(cost_function_history)
print('10 Cubic PCS Error Rate: ', test_error2)
...
#cubic_model = SVC(random_state = 0, kernel = 'poly', degree = 3)

# RBF SVM Error Rate
cubic_model = SVC(random_state = 0, kernel = 'rbf')
cubic_model.fit(train_pca10, train_y)
preds = cubic_model.predict(test_pca10)
test_error3 = compute_test_error_svm(preds, test_y)
print('New Error Rate: ', test_error3)
