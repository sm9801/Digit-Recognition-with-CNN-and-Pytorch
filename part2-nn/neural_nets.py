...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        #output_layer_error = -1 * (y - activated_output)# TODO
        output_layer_error = np.multiply((activated_output - y), np.vectorize(output_layer_activation_derivative)(output))
        # first mistake: hidden_layer_error
        hidden_layer_error = np.multiply(np.transpose(np.multiply(output_layer_error, self.hidden_to_output_weights)),
                                         np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input))
        # second mistake: bias_gradients
        bias_gradients = hidden_layer_error# TODO
        # third mistake: hidden_to_output_weight_gradients, matrix multiplication order
        hidden_to_output_weight_gradients = np.multiply(hidden_layer_activation, output_layer_error)# TODO
